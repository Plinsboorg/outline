...
export class DocumentConverter extends BaseConverter {
   ...
   * @param options.extractFrontmatter Whether YAML frontmatter is returned as
   *   parsed data instead of being rendered into the body as a codeblock.
   *   Defaults to false; set true where the frontmatter carries document
  ...
  public static async convert(
    ...
    const { extractTitle = true, extractFrontmatter = false } = options;
    let doc: Node;
    ...
    if (html !== undefined) {
      ...
      let markdown = await this.convertToMarkdown(content, fileName, mimeType, {
        // Extraction wants the frontmatter as data, so it must survive the
        // conversion to markdown rather than becoming a codeblock there.
        renderFrontmatter: !extractFrontmatter,
      });
  ...
  private static async convertToMarkdown(
    ...
    fileName: string,
    mimeType: string,
    options: { renderFrontmatter?: boolean } = {}
  ): Promise<string> {
    const { renderFrontmatter = true } = options;
    let markdown: string;
    ...
    return renderFrontmatter ? this.processFrontmatter(markdown) : markdown;
  }
