...
Phase 6 is implemented — see `docs/document-databases-phase-6-tasks.md`.

Remaining from the upstream discussion
[outline/outline#8476](https://github.com/outline/outline/discussions/8476):
**formulas** (per-row computed properties, the one gap several commenters named
as blocking a Notion migration) and **rollup filtering/sorting**, which
`PropertyQueryHelper` still rejects because rollups are computed per page.
