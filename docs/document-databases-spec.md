...
> **Historical note.** This is the original design RFC, written before
> implementation. It is kept for the rationale in §8 (key decisions and
> tradeoffs), not as a description of the current code. The shipped
> architecture diverged in one major way: a database is a facet of a
> **document** (sharing its primary key), not a property schema on a
> **collection** as described below. See `docs/document-databases-phase-3-tasks.md`
> and `docs/document-databases-phase-6-tasks.md` for what was actually built.
